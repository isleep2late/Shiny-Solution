...
(function (root) {
  ...
  // Under ?g1selftest, ?g2selftest or ?wizselftest (tests/run-tests.sh drives the page headless) the mode lives in
  // memory only, like the Gen 1 and Gen 2 TID tabs' and the wizard's stores, so a self-test never flips a visitor's mode.
  var MEMORY_ONLY = !!(root.location && typeof root.location.search === "string" &&
    (root.location.search.indexOf("g1selftest") !== -1 || root.location.search.indexOf("g2selftest") !== -1 || root.location.search.indexOf("wizselftest") !== -1));
  var mem = null;
  var listeners = [];
