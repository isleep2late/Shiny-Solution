...
`tools/gen1-tid-tables/tidderive2.cpp`, built from `~/gambatte-core` - a checkout on the author's machine, in neither repository and not committed - with the same g++ line; that directory has the scripts and the plateau, release, finite-hold and agreement logs, but not the sweep logs, which were never shipped) on
...
> `~/Desktop/Red-WR-Practice/gen2-tid` (a folder on the author's machine, not committed in either
> repository; what ships from it is the generated `core/data/gen2-tid.json`): its `README.md`, `REVIEW.md`, the 10 primary CSVs and the 152
...
| LID 01001 = Kenya's fixed OT ID, which is why the glitchless route wants it | `pokegold/engine/pokemon/move_mon.asm:1` `DEF RANDY_OT_ID EQU 01001` (checked in `~/AI/pret/pokegold`, an uncommitted local checkout of the public pret repository) | STRUCTURAL |
...
and [pret/pokefirered](https://github.com/pret/pokefirered) at the checkouts under `~/AI/pret/` (uncommitted
local clones of those public repositories - the GitHub links above are what a reader opens; 2026-09-02;
...
`tools/gen3-sid-savestate-harness`), over the RetroBridge mgx shim (`~/AI/Games/RetroBridge/embedded/shim`, `libmgx.so`, libmgba 0.10.5 - a build on the author's machine, not committed in either repository; the harnesses' results and logs are what ships),
...
**Status: EMPIRICAL throughout.** No Gen 5 decompilation exists (nothing under `~/AI/pret`, the
uncommitted local pret clones),
