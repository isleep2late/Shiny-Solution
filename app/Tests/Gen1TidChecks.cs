...
static class Gen1TidChecks
{
    ...
    public static int Run(string vectorsPath, string repoRoot, string? citationsPath = null)
    {
        ...
        // app/run-core-tests.sh passes a copy without the hold-START line as its negative control) ----
        {
            ...
            Check("verify carries the table and roll footnotes and its Sources", true, verify.Contains("  the table produces it at offset 358 [^2] [^4]") && verify.Any(l => l.StartsWith("  [^2] ") && l.Contains("(no decomp line; ")) && verify.Any(l => l.StartsWith("  [^4] pokered/engine/movie/oak_speech/init_player_data.asm:1-10 (docs/FACTS.md: Gen 1/2 (Game Boy) / Gen 1 Trainer ID / Where the ID comes from): InitPlayerData2")) && verify.Count(l => l.StartsWith("  [^")) == 2);
