...
CITATIONS = [
    ...
    # --- review round: Feebas map roll (D7), Bug Contest ability suppression, Gen 4 Everstone gate, dead two-Ditto flips
    ("pokeemerald", "src/wild_encounter.c", 121, "if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(MAP_ROUTE119)"),
    ("pokeemerald", "src/wild_encounter.c", 122, "&& gSaveBlock1Ptr->location.mapNum == MAP_NUM(MAP_ROUTE119))"),
    ("pokeruby", "src/wild_encounter.c", 85, "&& gSaveBlock1.location.mapNum == MAP_NUM(MAP_ROUTE119))"),
    ("pokeplatinum", "src/overlay006/wild_encounters.c", 407, "if (MapHeader_HasFeebasTiles(fieldSystem->location->mapId) && PlayerAvatar_IsFacingFeebasTile(fieldSystem)) {"),
    ("pokeplatinum", "src/map_header.c", 196, "return headerID == MAP_HEADER_MT_CORONET_B1F;"),
    ("pokeheartgold", "src/field/encounter_check.c", 920, "if (DoesAbilitySuppressEncounter(encounterGen, leadMon, level)) {"),
    ("pokeheartgold", "src/field/encounter_check.c", 966, "if (DoesAbilitySuppressEncounter(encounterGen, leadMon, level) || EncounterGen_DoesRepelSuppressEncounter(level, encounterGen) == TRUE) {"),
    ("pokeheartgold", "src/field/encounter_check.c", 976, "static BOOL FieldSystem_GenerateBugContestEncounter_Internal(FieldSystem *fieldSystem, Pokemon *leadMon, int rodType, EncounterGenState *encounterGen, u8 encType, int battler, BattleSetup *battleSetup) {"),
    ("pokeheartgold", "src/field/encounter_check.c", 978, "if (EncounterGen_DoesRepelSuppressEncounter(encSlot->maxLevel, encounterGen) == TRUE) {"),
    ("pokeplatinum", "src/overlay005/daycare.c", 335, "if (BoxPokemon_GetValue(boxMon[slot], MON_DATA_HELD_ITEM, NULL) == ITEM_EVERSTONE) {"),
    ("pokeplatinum", "src/overlay005/daycare.c", 336, "if (LCRNG_Next() >= (0xffff / 2)) {"),
    ("pokeheartgold", "src/get_egg.c", 235, "if (num_everstones == 2) {"),
    ("pokeheartgold", "src/get_egg.c", 236, "if (LCRandom() % 2 == 0) {"),
    ("pokeheartgold", "src/get_egg.c", 241, "if (LCRandom() >= 0x7FFF) {"),
    ("pokeheartgold", "src/get_egg.c", 247, "if (LCRandom() >= 0x7FFF) {"),
    ("pokeemerald", "src/daycare.c", 437, "if (dittoCount == DAYCARE_MON_COUNT)"),
    ("pokeplatinum", "src/overlay005/daycare.c", 326, "if (dittoCount == NUM_DAYCARE_MONS) {"),
    ("pokeemerald", "src/daycare.c", 1039, "if (eggGroups[0][0] == EGG_GROUP_DITTO && eggGroups[1][0] == EGG_GROUP_DITTO)"),
    ("pokeplatinum", "src/overlay005/daycare.c", 836, "if (eggGroups[0][0] == EGG_GROUP_DITTO && eggGroups[1][0] == EGG_GROUP_DITTO) {"),
    ("pokeheartgold", "src/get_egg.c", 687, "if (eggGroups[0][0] == EGG_GROUP_DITTO && eggGroups[1][0] == EGG_GROUP_DITTO) {"),
...
POKEFINDER = [
    ...
    ("Core/Gen3/Generators/WildGenerator3.cpp", "bool feebas = area.feebasLocation(profile.getVersion()) && feebasTile;"),
    ("Core/Gen4/Generators/WildGenerator4.cpp", "bool feebas = area.feebasLocation(profile.getVersion()) && feebasTile;"),
    ("Core/Gen3/Generators/EggGenerator3.cpp", "static bool compare(const EggState3 &left, const EggState3 &right)"),
]
