...
| **Gen 1 Trainer ID** R/B/Y | `core/gen1tid.js` / `Gen1Tid.cs` (engine merged from [RNG Solution](https://github.com/isleep2late/RNG-Solution); UI next): the one timed A press on NEW GAME under a named hold-START methodology, cue schedules from the menu / power-on / reset anchor, typed-TID calibration with the outlier and duplicate guards, route-valid targets (the `$40xx` sled, PSR sets), the save-corruption reset metronome, moderator `verify`, the runner's press-jitter model, and the Emerald/FRLG typed-TID -> Secret ID search. Tables emulator-derived on gambatte-core (GBA-silicon and DMG boots) | Red: GBA HD 5/5, DMG 5/6 hardware-validated; Blue and Yellow emulator-derived, no hardware sample yet (see [docs/FACTS.md](docs/FACTS.md), "Gen 1 Trainer ID") |
...
| `core/gen1tid.js` + `core/data/gen1-tid.json` + `core/data/gen3-sid.json` | The Gen 1 Trainer ID engine and its data (tables, methodologies, reset models, target sets; the Gen 3 Secret ID models), ported from RNG Solution's Python and checked bit-for-bit against vectors it emits (`tests/gen1tid-vectors.json`). The data files are generated by `tools/gen-gen1-data.py` from RNG Solution's registry and are embedded in the C# engine. |
...
- `tests/test-gen1tid.cjs` and `app/Tests --gen1tid` — the JS and C# Gen 1 Trainer ID / Gen 3
  Secret ID / press-jitter engines vs `tests/gen1tid-vectors.json`, emitted by RNG Solution's Python
  (`tests/emit_vectors.py` there); both runners also show a corrupted vector failing.
