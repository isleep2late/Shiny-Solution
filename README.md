...
| **Gen 1 Trainer ID: Blue** | same | same, except that Blue's `$40xx` relaxation is **inferred from Red**, not measured on Blue | `blue/gba/hold-start-v1`, `blue/dmg/hold-start-v1`: emulator-derived (every offset re-derived from three further cold boots, byte-identical); **no hardware sample yet**. The `$40xx` rule here is **inferred from Red** — byte-identical bank `$1D`, same swap source — and was **never swept on Blue**; like Red's it is emulator-only and unconfirmed on a cartridge |
