...
| **Wanted-IVs wizard: Gen 3 / Gen 4** (the web app's **Wizard (Gen 3/4)** tab, `webapp/wizard-ui.js` over `core/generators.js`, `core/seedtime4.js`, `core/timers.js` and `core/data/{species,encounters,statics}-gen{3,4}.json`, [USAGE.md](USAGE.md)) | Game -> console -> a static / gift from the catalogue or a wild slot from the encounter tables (time of day on Gen 4; Emerald and Gen 4 lead effects) -> per-stat IV ranges, nature, ability slot, gender where the species allows, shiny with the typed IDs -> the matching frames from the game's seed model: Ruby / Sapphire dead-battery `0x5A0`, Emerald `0`, FireRed / LeafGreen a typed seed (else stated unavailable), a Gen 4 seed a DS clock can reach (the IV back-step, the hour-byte filter, every candidate re-generated by the wild or static generator) -> the result card (PID, IVs, nature, gender, ability, Hidden Power, stats, shiny yes / no, frame, seed, the seed model with its validation status) -> a numbered procedure (the clock setting and minutes-before rule, EonTimer's phases, the coin-flip / Elm-call verification, the advance plan) -> the page's countdown -> the typed outcome (nature and IVs or stats on Gen 3; coin flips or Elm calls on Gen 4) inverted to the frame or delay hit and the calibration moved, per game, console, seed model and mode | Frames are stated as times at the console's rate; a request with no frame inside the horizon (an hour by default) is answered with the honest limit: the expected count per 100,000 frames (exact over the LCRNG cycle when affordable) and, for statics, the exact first frame of the whole cycle (a flawless Emerald mon: frame 176,562,488, 34.2 days) | Every output is **EMPIRICAL / model output**: no hardware session has run any wizard procedure; the seed models are `rs/gba/boot-seed-v0`, `emerald/gba/boot-seed-0-v0`, `frlg/gba/typed-seed-v0`, `dppt/nds/seed-to-time-v0`, `hgss/nds/seed-to-time-v0`, each printed with its status. Checked against the generator vectors (Groudon Method 4 frame 3, Route 111 frame 7, Route 222 Magnet Pull seed `5D1745D0` frame 0) and the design's gate seed `7B0448D1` -> frame 0 |
...
| `core/data/{species,encounters,statics}-gen{3,4}.json` | The Gen 3/4 species tables, wild encounter tables and static/gift catalogue generated from the pret decompilations by `tools/gen-guide-data.py` (`docs/DATA.md`): the records the generator engines take as input, read by the web app's Wizard tab (loaded lazily on first use from `webapp/data/wizard-gen{3,4}.js`, which `sync-core.sh` writes; not in the mobile bundle, `docs/DATA.md`). |
...
- `tests/run-tests.sh`: the RUN / PRACTICE-HUNT wall (`test-mode-wall.cjs` over
  ...
  seed, eggs, the fixed-PID Pichu, the combination cap) and the store split by mode and seed model)
...
- `app/run-core-tests.sh`: the C# engine vs the same vectors, plus canonical MT19937 vectors,
  ...
  failing; `tests/run-tests.sh` re-emits that file from the tab and requires it byte-identical. `dotnet build app/App -c Release -p:EnableWindowsTargeting=true`
...
- **Gen 4**: one DS session to record the two-phase timer's calibration on hardware, and to land
  one seed chosen by the seed-to-time layer through the Wizard tab (its hardware gate: the tab's
...
- **Gen 3 wizard on a console**: the Wizard tab's Emerald / dead-battery Ruby procedures have no
  hardware record either; one static hit typed back (nature and IVs) is the first sample. Gate: an
