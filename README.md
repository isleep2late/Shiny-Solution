...
| `core/data/citations.json` | The decomp citation registry generated from `docs/FACTS.md` by `tools/gen-citations.py` (each citation read in pret: the section of FACTS.md, the line, the cited line's text; `docs/DATA.md`). The wizard's procedure in the web tab and the desktop panel marks each step's sources `[^n]` and lists them after the steps over this registry: a decomp line under its FACTS.md section, or SYNTHESISED where the source is a timer model, a measured constant or a community convention with no decomp line; a citation the registry lacks is printed as NOT IN THE REGISTRY and refused by the tests. |
...
- The citation registry (in `run-tests.sh`): `core/data/citations.json` regenerated from `docs/FACTS.md` and pret and
  ...
  reported as NOT IN THE REGISTRY; the browser self-test does the same through the page; `app/run-core-tests.sh` gives
  the C# check a registry without that line and shows it failing.
- `tests/test-sw.cjs` (in `run-tests.sh`): `webapp/sw.js` driven in a node sandbox with fake caches, fetch and clients:
  ...
  the shell, any navigation under the scope and a wizard table opened once are served from the caches while a table
  never opened is not; the build stamp in `sw.js` equals the one in `index.html` and the hash the shipped sources give
