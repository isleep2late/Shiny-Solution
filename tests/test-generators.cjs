...
// ---------------------------------------------------------------- rarity and reachability
{
  ...
  // nine natures only
  const natures = new Set(flawless1.map((s) => s.nature));
  check("flawless PIDs cover only a few natures", natures.size <= 9, true);
}
{
...
if (fs.existsSync(crossPath)) {
  ...
  for (const c of X.cases) {
    ...
    else if (c.kind === "egg4") r = G.gen4Egg(Object.assign(common, { seed: c.seed, seedPickup: c.seedPickup, game: c.game, species: c.species, parentIvs: c.parentIvs, masuda: c.masuda, everstoneNature: c.everstoneNature, pickupCount: c.frameCount }));
    else if (c.kind === "egg3e") r = G.gen3EggEmerald(Object.assign(common, { seed: c.seed, method: c.method, calibration: c.calibration, compatibility: c.compatibility, species: c.species, parentIvs: c.parentIvs, parentGenders: c.parentGenders, parentItems: c.parentItems, parentNatures: c.parentNatures, pickupCount: c.frameCount }));
    ...
    const got = r.map((x) => x.valid === false ? { frame: x.frame, valid: false } : { frame: x.frame, pid: x.pid, ivs: x.ivArray, level: x.level, slot: x.encounterSlot === undefined ? -1 : x.encounterSlot, form: x.form === undefined ? 0 : x.form, adv: x.advances === undefined ? -1 : x.advances, pick: x.pickupAdvances === undefined ? -1 : x.pickupAdvances, inh: x.inheritanceArray || null, valid: true });
