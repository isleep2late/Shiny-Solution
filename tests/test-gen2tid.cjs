...
for (const c of V.sampleFromHit) {
  ...
  if (r.sample) {
    // the Gen 1 mean and duplicate helpers accept the record as it is (they read implied_ms, tid, aimed and attempt); the
    // outlier guard is the bin one (gen1tid.js addSample checks whole-frame offsets, and hit / aimed here are bin centres)
    check(`sampleFromHit ${c.game} feeds gen1 meanCorrection`, r.sample.implied_ms, G1.meanCorrection([r.sample], 0));
    check(`sampleFromHit ${c.game} is not a duplicate of an empty store`, false, G1.isDuplicate([], r.sample));
    check(`sampleFromHit ${c.game} within the outlier guard`, false, G.isOutlierBins(G.binRule(DATA, c.game), r.sample.hit_bin, r.sample.aimed_bin));
  }
}
